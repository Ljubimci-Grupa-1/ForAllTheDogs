...
const UserProfile = ({isLoggedIn, userEmail, currentUser}:UserProfileProps) => {
    ...
    useEffect(() => {
        ...
        fetch('http://localhost:8080/ad/all')
            ...
            .then((data) => {
                ...
                for (let i = 0; i < petsData.length; i++) {
                    if (petsData[i].user.email===userEmail) {
                        console.log("j")
                        filtrirani.push(petsData[i]);
                    }
                }
                const separatedArrays = filtrirani.reduce<{
                    activeAds: LostPet[];
                    inactiveAds: LostPet[];
                }>(
                ...
                console.log("korisnikoiiiiiiiiiiii", petsData)
